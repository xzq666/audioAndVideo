...
+ (void)addThemeTrigger:(id)target{
    ...
    if ([NSStringFromClass([target class]) isEqualToString:@"CYLTabBar"]) {
        targetName = [NSStringFromClass([target class]) substringFromIndex:3];
    }
    ...
    if ([NSStringFromClass([target class]) isEqualToString:@"CYLTabBar"]) {
        themeClassStr = [NSString stringWithFormat:@"ZX%@Theme",[NSStringFromClass([target class]) substringFromIndex:3]];
    }
    Class themeClass = NSClassFromString(themeClassStr);
    if(themeClass){
        u_int proCount;
